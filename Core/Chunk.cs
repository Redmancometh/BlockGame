...
public class Chunk : MonoBehaviour {
	...
	public void LateUpdate () {
		...
		// timeout
		if (MasterEngine.getInstance().EnableChunkTimeout && EnableTimeout) {
			Lifetime += Time.deltaTime;
			if (Lifetime > MasterEngine.getInstance().ChunkTimeout) {
				FlaggedToRemove = true;
			}
		}
	
		if (FlaggedToUpdate && VoxelsDone && !DisableMesh && MasterEngine.getInstance().GenerateMeshes) { // check if we should update the mesh
		...
		if (FlaggedToRemove) {
			...
			if (MasterEngine.getInstance().SaveVoxelData) { // save data over time, destroy chunk when done
				if (ChunkDataFiles.SavingChunks == false) { // only destroy chunks if they are not being saved currently
					if (ChunkManager.SavesThisFrame < MasterEngine.getInstance().MaxChunkSaves) {
	...
	private void SaveData () {	
		...
		if (MasterEngine.getInstance().SaveVoxelData == false) {
			Debug.LogWarning ("StarColony: Saving is disabled. You can enable it in the Engine Settings.");
			return;
		}
	...
	IEnumerator RequestVoxelData () { // waits until we're connected to a server and then sends a request for voxel data for this chunk to the server
		...
		while (MasterEngine.getInstance().MaxChunkDataRequests != 0 && Chunk.CurrentChunkDataRequests >= MasterEngine.getInstance().MaxChunkDataRequests) {
			yield return new WaitForEndOfFrame();
		}
		
		Chunk.CurrentChunkDataRequests ++;
		MasterEngine.getInstance().UniblocksNetwork.GetComponent<NetworkView>().RPC ("SendVoxelData", RPCMode.Server, Network.player, ChunkIndex.x, ChunkIndex.y, ChunkIndex.z);
	}
