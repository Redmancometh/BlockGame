...
public class ChunkManager : MonoBehaviour {
	...
	void Start () {
		targetFrameDuration = 1f / MasterEngine.getInstance().TargetFPS;
		
		ChunkManager.Chunks = new Dictionary<string, Chunk>();
		ChunkManager.ChunkUpdateQueue = new List<Chunk>();
		frameStopwatch = new Stopwatch ();
		
		MasterEngine.getInstance().ChunkScale = ChunkObject.transform.localScale;
	...
	public static GameObject SpawnChunk ( int x, int y, int z ) { // spawns a single chunk (only if it's not already spawned)
		...
		if (chunk == null) {
			return MasterEngine.getInstance().ChunkManagerInstance.DoSpawnChunk(new Index (x,y,z));
		}
		else return chunk;
	}
	...
	public static void SpawnChunks ( float x, float y, float z) { // take world pos, convert to chunk index
		Index index = MasterEngine.getInstance().PositionToChunkIndex (new Vector3(x,y,z));
		MasterEngine.getInstance().ChunkManagerInstance.TrySpawnChunks(index);
	}
	public static void SpawnChunks ( Vector3 position ) {
		Index index = MasterEngine.getInstance().PositionToChunkIndex (position);
		MasterEngine.getInstance().ChunkManagerInstance.TrySpawnChunks(index);
	}
	
	public static void SpawnChunks ( int x, int y, int z ) { // take chunk index, no conversion needed
		MasterEngine.getInstance().ChunkManagerInstance.TrySpawnChunks (x,y,z);
	}
	public static void SpawnChunks ( Index index ) {
		MasterEngine.getInstance().ChunkManagerInstance.TrySpawnChunks (index.x, index.y, index.z);
	}
	...
	private void StartSpawnChunks ( int originX, int originY, int originZ ) {
		...
		int range = MasterEngine.getInstance().ChunkSpawnDistance;
