...
public class ChunkMeshCreator : MonoBehaviour {
				...
				while (z < SideLength) {
					...
					if ( voxel != 0 ) { // don't render empty blocks.
						...
						if ( voxelType.VCustomMesh == false ) { // if cube
							...
							if (colliderType == ColliderType.none && MasterEngine.getInstance().GenerateColliders) {
	...
	private bool CheckAdjacent ( int x, int y, int z, Direction direction, Transparency transparency ) { // returns true if a face should be spawned
		...
		if (adjacentVoxel == ushort.MaxValue) { // if the neighbor chunk is missing
			...
			if (MasterEngine.getInstance().ShowBorderFaces || direction == Direction.up) {
				return true;
			}
			else {
				return false;	
		...
		Transparency result = MasterEngine.getInstance().GetVoxelType (adjacentVoxel).VTransparency;	// get the transparency of the adjacent voxel
	...
	public bool CheckAllAdjacent ( int x, int y, int z ) { // returns true if all adjacent voxels are solid
		...
		for (int direction=0; direction<6; direction++) {
			if ( MasterEngine.getInstance().GetVoxelType ( chunk.GetVoxel (chunk.GetAdjacentIndex(x,y,z, (Direction)direction)) ).VTransparency != Transparency.solid ) {
				return false;
			}
		}
		return true;
	...
	private void CreateFace ( ushort voxel, Facing facing, ColliderType colliderType, int x, int y, int z ) { 
		...
		if (facing == Facing.forward) {
			...
			Vertices.Add(new Vector3(x- 0.5001f, y- 0.5001f, z+ 0.5f));
			Vertices.Add(new Vector3(x+ 0.5001f, y- 0.5001f, z+ 0.5f)); 
			if (colliderType == ColliderType.cube && MasterEngine.getInstance().GenerateColliders) {
		...
		// ==== UVs =====
		float tUnit = MasterEngine.getInstance().TextureUnit;
		Vector2 tOffset = MasterEngine.getInstance().GetTextureOffset (voxel, facing); // get texture offset for this voxel type
		
		float pad = tUnit * MasterEngine.getInstance().TexturePadding;
