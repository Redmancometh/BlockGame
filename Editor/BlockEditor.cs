...
public class BlockEditor : EditorWindow {
	...
	private bool FindEngine () { // returns false if engine not found, else true
		...
		foreach (Object obj in Object.FindObjectsOfType<MasterEngine>()) {
			if (obj != null) {
				MasterEngine engine = obj as MasterEngine;
	...
	[MenuItem ("Window/StarColony: Block Editor")]
	static void Init () {
	...
	private void SelectBlock ( GameObject block ) {
		...
		try {
			...
			Debug.LogError ("StarColony: Cannot find the block prefab! Have you deleted the block 0 prefab?");
		}
	...
	private string GetPath () {
		try {
			return EngineInstance.GetComponent<MasterEngine>().lBlocksPath;
		}
		catch (System.Exception) {
