...
public class UniblocksClient : MonoBehaviour {
	...
	public void OnConnectedToServer () {
		...
		if (MasterEngine.getInstance().EnableMultiplayer == false) 	Debug.LogWarning ("StarColony: Multiplayer is disabled. Unexpected behavior may occur.");
		MasterEngine.getInstance().SaveVoxelData = false; // disable local saving for client
	}
	...
	public static void UpdatePlayerPosition (int x, int y, int z) {
		MasterEngine.getInstance().UniblocksNetwork.GetComponent<NetworkView>().RPC ("UpdatePlayerPosition", RPCMode.Server, Network.player, x, y, z);
	}
	public static void UpdatePlayerPosition (Index index) {
		MasterEngine.getInstance().UniblocksNetwork.GetComponent<NetworkView>().RPC ("UpdatePlayerPosition", RPCMode.Server, Network.player, index.x, index.y, index.z);
	}
	public static void UpdatePlayerRange (int range) {
		MasterEngine.getInstance().UniblocksNetwork.GetComponent<NetworkView>().RPC ("UpdatePlayerRange", RPCMode.Server, Network.player, range);
	}
