...
public class UniblocksServer : MonoBehaviour {
	...
	void Awake () {
		...
		MasterEngine.getInstance().UniblocksNetwork = this.gameObject;
	...
	void OnPlayerDisconnected (NetworkPlayer player) {
		if (MasterEngine.getInstance().MultiplayerTrackPosition) {
	...
	void DistributeChange ( NetworkPlayer sender, int x, int y, int z, int chunkx, int chunky, int chunkz, int data, bool isChangeBlock ) { // sends a change in the voxel data to all clients
		...
		foreach (NetworkPlayer player in Network.connections) {
			...
			if (MasterEngine.getInstance().MultiplayerTrackPosition==false || IsWithinRange (player, new Index (chunkx,chunky,chunkz))) { // check if the change is within range of each player
	...
	void Update () {
		...
		if (AutosaveTime > 0.0001f) {
			if (autosaveTimer >= AutosaveTime) {
				autosaveTimer = 0;
				MasterEngine.getInstance().SaveWorld();
			}
			else {
